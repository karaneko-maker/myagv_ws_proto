...
class MoveControllNode(Node):

    def __init__(self):
        super().__init__('move_controll_node')
        self._action_client = ActionClient(self, NavigateToPose, 'navigate_to_pose')
        self.publisher_ = self.create_publisher(String, '/ans/chatter', 10)
    ...
    def get_result_callback(self, future):
        ...
        if result == 4:  # 4 is the status code for succeeded
            self.get_logger().info(f'ゴールに到着しました {self.current_destination}')
